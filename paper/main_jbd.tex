...
\item We introduce a popularity-aware prompt variant and show that explicit popularity metadata partially restores judge sensitivity for two representative judges: consistency rises from 0.262 to 0.550 for Qwen2.5-7B-Instruct (local) and from 0.542 to 0.810 for DeepSeek-V3, with substantial gains in coverage-, tail-, Gini-, and ARP-based alignment for the stronger judge.
\item We package the protocol, benchmark artifacts, and analysis scripts as a reusable toolkit for stress-testing whether pairwise judges remain sensitive to known metric trade-offs in recommendation.
\end{enumerate}
...
\noindent\textbf{Metric alignment.}  For each pair, the \emph{LLM winner} is the model preferred by the majority of consistent users. We compute task-level alignment as 1.0 if the LLM winner matches the offline metric winner, 0.5 if either side is tied, and 0.0 otherwise. For metrics where higher is better (NDCG, Coverage, Tail Exposure), the metric winner is the model with the larger offline value; for metrics where lower is better (Gini, ARP), the metric winner is the model with the smaller offline value. In the blind six-judge comparison, we report NDCG and Coverage because they form the main accuracy--distribution trade-off shared across all judges. In the popularity-aware probe, we additionally report alignment to Tail Exposure, Gini, and ARP using the same task-level scoring rule.
...
These results refine the core claim of the paper. The blind protocol does not merely reveal weak judges; it appears to obscure the distributional evidence that debiasing methods are designed to improve. Once popularity metadata is made explicit, at least some judges recover substantial sensitivity, although not enough to displace offline metrics as the primary evidence for long-tail claims.
...
The popularity-aware probe makes this causal story harder to dismiss. Once HEAD/MID/TAIL evidence is exposed, Qwen2.5-7B-Instruct (local) becomes materially more stable and DeepSeek-V3 becomes substantially more aligned with coverage, tail exposure, Gini, and ARP. The blind protocol is therefore not a neutral presentation choice; it appears to obscure the very signal that a debiasing-sensitive judge would need to use.
...
Our constructive probe matters because it breaks the pure negative-result framing. The local 7B judge becomes markedly more stable, and DeepSeek-V3 becomes substantially more aligned with the distributional metrics that matter for debiasing once popularity evidence is made explicit. This is exactly what we would expect if the blind protocol obscures the target signal rather than if LLM judges were fundamentally incapable of distributional reasoning.
